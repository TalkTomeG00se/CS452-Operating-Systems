...
int tsb_getSize(struct tsb_list * list) {
    ...
    return getSize(list->list); // using the getSize function on the list
...
Boolean tsb_isFull(struct tsb_list * list) {
    ...
    return tsb_getSize(list) >= tsb_getCapacity(list); // relational operator, comparing size to current capacity. True if full, else no
...
NodePtr tsb_removeNode(struct tsb_list * list, NodePtr node) {
    ...
    if(list->stop_requested == 0){ // if stop has not been requested
        ...
        temp = removeNode(list->list, temp); // set temp to the removed node from the front, per existing function
    }
...
void tsb_finishUp(struct tsb_list * list) {
   ...
   while(tsb_isEmpty(list) == 0){} // while list is not empty, do nothing
